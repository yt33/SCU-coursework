 ...
 */
  ...
  for (q=0; q < NR_SCHED_QUEUES; q++) {	
	...
	{
		ticket = rand() % 50;
		if (ticket == 0) // changes q only every 50 processes
		{
			q  = rand() % NR_SCHED_QUEUES; // to implement randomness, q is reassigned to a random number between 0 and NR_SCHED_QUEUES
			if(!(rp = rdy_head[q])) {
