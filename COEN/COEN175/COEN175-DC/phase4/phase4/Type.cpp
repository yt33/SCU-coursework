...
{
    if(_specifier == CHAR && _indirection == 0 && _kind == SCALAR) //dont promote a char pointer to an int pointer
        return Type(INT);
    if(_kind == ARRAY)
...
{
    return (this->promote().indirection() >= 1);
    //return (this->promote().indirection() >= 1);
    //return (_indirection >= 1 && _kind == SCALAR);
}

bool Type::isInteger() const 
{
    //return (this->promote().specifier() == INT);
    return _indirection == 0 && _specifier == INT;
}

bool Type::isNumeric() const 
{
    return (this->promote().indirection() == 0 && (this->promote().specifier() == INT || this->promote().specifier() == DOUBLE));
    //return (this->promote().specifier() == INT || this->promote().specifier() == DOUBLE);
    //return(_indirection == 0 && (_specifier == INT || _specifier ==  DOUBLE));
    //return (_specifier == INT || _specifier == DOUBLE);
}

bool Type::isPredicate() const { //Returns false ONLY for functions. Everything else is promoted to either numeric or predicate
    //isNumeric() || isPointer()
    return _kind != FUNCTION && _kind != ERROR;
}
...
bool Type::isCompatibleWith(const Type &that) const 
{
    ...
    return (isPredicate() && *this == that); //Both numeric or identical predicate types
}
